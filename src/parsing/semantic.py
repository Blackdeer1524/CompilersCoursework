from .parser import (
    Program, Function, Argument, Statement, Assignment, Reassignment,
    Condition, ForLoop, UnconditionalLoop, FunctionCall, Return, Break, Continue, Block, Expression,
    BinaryOp, UnaryOp, Identifier, IntegerLiteral, CallExpression
)
...
class FunctionInfo:
    ...
    def __init__(self, name: str, return_type: str, params: list[tuple[str, str]], line: int, column: int):
        self.name = name
...
class SymbolTable:
    ...
    def declare_variable(self, name: str, var_type: str, line: int, column: int):
        """Declare a variable in the current scope."""
        if name in self.variables:
            raise SemanticError(f"Variable '{name}' already declared in this scope", line, column)
...
class SemanticAnalyzer:
    ...
    def __init__(self, program: Program):
        ...
        self.loop_depth: int = 0  # Track loop nesting depth for break/continue validation
    ...
    def _collect_function(self, func: Function):
        ...
        param_types = [(arg.name, arg.type) for arg in func.args]
        func_info = FunctionInfo(func.name, func.return_type, param_types, func.line, func.column)
    ...
    def _analyze_function(self, func: Function):
        ...
        for arg in func.args:
            try:
                self.current_scope.declare_variable(arg.name, arg.type, func.line, func.column)
            except SemanticError as e:
                self.errors.append(e)
    ...
    def _analyze_assignment(self, stmt: Assignment):
        ...
        if value_type != stmt.type:
            self.errors.append(SemanticError(
                f"Type mismatch: cannot assign {value_type} to variable '{stmt.name}' of type {stmt.type}",
                stmt.line,
                stmt.column
            ))
        ...
        try:
            self.current_scope.declare_variable(stmt.name, stmt.type, stmt.line, stmt.column)
        except SemanticError as e:
            self.errors.append(e)
    ...
    def _analyze_reassignment(self, stmt: Reassignment):
        ...
        var_type = self.current_scope.lookup_variable(stmt.name)
        if var_type is None:
            self.errors.append(SemanticError(
                f"Variable '{stmt.name}' is not declared",
                stmt.line,
                stmt.column
            ))
            return
    ...
    def _analyze_expression(self, expr: Expression) -> str:
        ...
        match expr:
            ...
            case Identifier():
                ...
                if var_type is None:
                    ...
                    self.errors.append(SemanticError(
                        f"Variable '{expr.name}' is not declared",
                        line, column
                    ))
    ...
    def _analyze_binary_op(self, expr: BinaryOp) -> str:
    ...
    def _analyze_unary_op(self, expr: UnaryOp) -> str:
    ...
    def _analyze_call_expression(self, expr: CallExpression) -> str:
        ...
        return "int"  # Default for error recovery
    
    def _check_function_call(self, name: str, args: list[Expression], line: int, column: int) -> FunctionInfo | None:
        ...
        for i, (arg_expr, (param_name, param_type)) in enumerate(zip(args, func_info.params)):
